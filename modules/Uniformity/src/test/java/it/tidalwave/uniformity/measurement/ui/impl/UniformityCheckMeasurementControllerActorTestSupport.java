...
//            add(new ActorActivator(SpotReadActor.class, 1)); // to test the real thing
    ...
    public void must_properly_drive_a_complete_3x3_sequence() 
      throws InterruptedException
      {
        ...
        inOrder.verify(actions.action("Continue")).setEnabled(             eq(false));
        inOrder.verify(actions.action("Cancel")).setEnabled(               eq(false));
        ...
        inOrder.verify(actions.action("Continue")).setEnabled(             eq(true));
        inOrder.verify(actions.action("Cancel")).setEnabled(               eq(true));
        inOrder.verify(actions.action("Continue")).actionPerformed(any(ActionEvent.class));
        inOrder.verify(actions.action("Continue")).setEnabled(             eq(false));
        inOrder.verify(actions.action("Cancel")).setEnabled(               eq(false));
